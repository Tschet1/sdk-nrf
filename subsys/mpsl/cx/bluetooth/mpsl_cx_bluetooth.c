...
#if DT_HAS_COMPAT_STATUS_OKAY(sdc_radio_coex_three_wire)
#define COEX_NODE DT_COMPAT_GET_ANY_STATUS_OKAY(sdc_radio_coex_three_wire)
#else
#define COEX_NODE DT_INVALID_NODE
#error No "sdc-radio-coex-three-wire"-compatible (and enabled) nodes found
#endif
...
int mpsl_cx_bt_interface_3wire_config_set(void)
{
	...
	gpiote_cfg->gpio_pin = DT_GPIO_PIN(COEX_NODE, req_gpios);
	...
	gpiote_cfg->gpio_pin = DT_GPIO_PIN(COEX_NODE, pri_dir_gpios);
	...
	gpiote_cfg->gpio_pin = DT_GPIO_PIN(COEX_NODE, grant_gpios);
